...
| 2026-08-23（新增「今日运势」`generate_liuri_reading()`——每日任务系统用的轻量流日AI解读，独立于本节六步深析`analyze_bazi()`流水线，不共享缓存、不影响六步任何既有逻辑） | 用户新需求：基于"流日"（今天的干支跟命盘四柱产生的十神/生克/刑冲合会关系）每天生成一段简短AI解读，作为每日任务之一。前置条件：`js/bazi-engine.js`新增`BaziEngine.getLiuri(year,month,day)`（内部`_liuri()`，仿`_liuyue()`同一套`Solar.fromYmd→getLunar→getEightChar`接口取日柱干支，刻意不接入`calculate()`主流程——流日粒度是"天"，塞进按出生信息永久缓存的`calculate()`返回值会破坏"命盘计算结果永久有效"这一既有假设）+`BaziEngine.getLiuriRelations(baziData, liuri)`（只判定"流日 vs 命盘四柱"的冲/合/三刑子卯/害/破，不判定流日跟自己/流年/流月的关系，不判定三合三会三刑三支局/自刑——这些需要"三支皆现"或命盘内部重复出现的整组上下文判定，超出流日与命盘互动关系的语义范围）。为避免"流日跟四柱的关系"和"四柱内部关系"变成两份独立实现容易分叉，把`_interactions()`原本内联的冲/六合/三刑子卯/六害/六破判定表+单对判定逻辑提取成类级共享静态方法`_pairZhiTables()`/`_pairwiseZhiRelations(a,b)`，两个消费方共用同一份判定表 | `gemini_analysis.py`新增`generate_liuri_reading(bazi_data, liuri, relations, gender, birth_year, force_refresh)`，沿用六步深析同一套`_call_gemini()`/`_parse_json()`/`PERSONA_SYSTEM`人设与错误处理模式（不另起一套风格，`_redact()`同款Key脱敏、`GeminiCallError`同款诊断信息）；`max_tokens=2048`（远小于六步任何一个narrative字段，目标正文只有100-150字）。prompt要求：贴近日常生活语气、不堆砌"十神""冲合刑害"命理术语本身、给具体"今天可以关注/留意"的感受而非"诸事顺利"空泛套话、不制造焦虑不做绝对化承诺（延续`PERSONA_SYSTEM`已定下的"不故弄玄虚/不说正确废话"立场）。**架构关键点——独立缓存维度**：新增`LIURI_CACHE`（`./persistent_data/liuri_cache/`，与六步深析`ANALYSIS_CACHE`完全独立的目录）；缓存key`_liuri_cache_key(bz_hash, date_str)`是"命盘hash + 日期字符串"二维组合（六步深析只有"命盘hash"一维、同一命盘永久缓存有效，流日必须多一维日期才不会把"今天"的内容永久缓存成"这张命盘"的答案），`date_str`固定用`_today_utc_str()`（服务端`datetime.now(timezone.utc)`推导），不信任客户端传来的日期字段，避免客户端本地时钟偏差/篡改影响缓存维度的权威性。`main.py`新增`POST /liuri-reading`端点+`LiuriRequest`请求模型，沿用`/analyze-bazi`同一套请求校验/`await`异步调用/503错误处理风格；请求体含前端已算好的`liuri`/`relations`（后端只消费不重算，跟`bazi_data`其它字段一贯的"JS引擎算一次、Python侧只消费"分工原则一致） | **JS重构等价性**：用真实`lunar-javascript`库+78组覆盖1950-2025年不同年/月/性别的命盘参数，逐条比对`_interactions()`重构前后的`interactions`字段输出，0处mismatch，确认提取共享判定表是字节级等价的纯重构，未影响六步深析/图像生成等下游已依赖`interactions`字段的任何现有逻辑。**流日干支正确性**：`getLiuri(2000,1,1)`→戊午、`getLiuri(2024,1,1)`→甲子，均与可查证的农历万年历日柱干支一致；2026年8月连续10天日柱按60甲子顺序正确递增无跳位。**关系判定**：手工构造命盘验证流日子对日支午命中"冲"、对时支卯命中"三刑"，文案与`_interactions()`共享判定表完全一致；十神计算（流日甲对日主甲→比肩）正确。**缓存隔离（本次重点）**：mock `_call_gemini()`，验证同一命盘同一天内二次请求命中缓存（零新增Gemini调用）；monkeypatch `_today_utc_str()`模拟"次日"，验证同一命盘hash在不同日期各自生成独立缓存文件、互不覆盖，"明天"请求正确判定未命中触发重新生成；验证`ANALYSIS_CACHE`目录全程未被触碰。错误路径（缺`reading`字段/非法JSON/无API Key）均返回明确`error`不崩溃。`node --check js/bazi-engine.js`、`python3 -m py_compile island_service/gemini_analysis.py island_service/main.py`全部通过。**待测试**——本地无真实`GEMINI_API_KEY`，`generate_liuri_reading()`真实生成内容的字数/语气质量需部署后用真实Key实测确认；前端UI展示/每日任务奖励逻辑由并行的user-system任务负责，接口contract（`/liuri-reading`请求/返回字段）是否对得上需qa-reviewer基于全部并行改动的真实diff一并复查 |
| 2026-08-23（qa-reviewer复查上一行「今日运势」缓存key设计，1个CONFIRMED时区错位bug修复） | qa-reviewer用真实场景复现：流日（今天的干支）是前端`js/bazi-engine.js::getLiuri()`按**用户本地时间**算出来再传给后端的，但服务端缓存key第二维度`_today_utc_str()`用的是`datetime.now(timezone.utc)`——这两个"今天"在马来西亚/中国等UTC+8时区的本地时间00:00-08:00这8小时窗口内必然不一致（服务端UTC仍是"昨天"，前端传来的干支已经是"今天"的）。实测复现：本地时间2026-08-23 01:00（+08区）时前端算出流日"己巳"，UTC仍是2026-08-22，服务端却用"昨天"的日期查/写缓存，返回真正前一天"戊辰"对应生成的内容——文不对题，且每天都会发生、不是偶发个案 | 根因是缓存key用了"服务端自己猜的日期"这个代理值，而不是真正决定内容对不对的输入本身（流日干支）——前端传来的干支本身就是"今天到底是哪天"这件事的权威答案，服务端不该自己另算一个日期去做缓存判断。**修复**：`_liuri_cache_key(bz_hash, date_str)`签名改为`_liuri_cache_key(bz_hash, ganzhi_str)`，直接用`liuri['gan']+liuri['zhi']`拼缓存key第二维度，不再依赖服务端时钟；`_today_utc_str()`改完后零调用点，已删除（连带清理不再使用的`datetime`/`timezone`导入）。新增`_is_valid_ganzhi(gan, zhi)`——60甲子合法性校验（天干地支index同奇偶，10×12=120种组合里恰好60种真实存在，非经验规则），`generate_liuri_reading()`入口先校验，非法组合（空字符串/乱码/阳干配阴支这类不存在的搭配）直接返回`error:'invalid_ganzhi(...)'`拒绝，不进入生成流程、不落盘垃圾缓存。`_liuri_cache_write()`落盘内容新增`ganzhi`字段（`{'reading':...,'ganzhi':'己巳'}`），此前只存`reading`本身、缓存文件完全没有自检线索能事后核对"这份缓存对应哪个干支"，现在能直接比对文件内`ganzhi`与请求实际传入是否一致定位问题。返回体字段`date`→`ganzhi`（该接口截至本次修复尚无前端调用方接入，`js/`下未找到`/liuri-reading`的fetch调用，改字段名无破坏既有契约风险）。`main.py::/liuri-reading`端点注释/docstring同步更新，反映新的缓存维度与返回字段 | mock `_call_gemini()`（隔离测试用临时`LIURI_CACHE`目录，未触碰真实缓存目录）验证三项场景：①同一命盘、同一真实流日干支"己巳"两次请求——第二次`from_cache:True`、reading与第一次完全一致、`_call_gemini`总调用次数为1（不因服务端系统时钟/UTC日期而分叉，因为改动后压根不读服务端时钟）；②同一命盘、不同流日干支"己巳"vs"戊辰"（模拟真正跨天）——两次请求各自生成独立缓存文件（`{hash}_己巳.json`/`{hash}_戊辰.json`）、内容不同、互不覆盖；③非法干支组合——"甲丑"（阳干配阴支，不存在的60甲子组合）、空字符串`''`+`''`、乱码`'X'`+`'Y'`三种输入均被正确拒绝（`reading:None`、`error`含`invalid_ganzhi`字样），且`_call_gemini`调用次数在这三次尝试后仍保持2（未额外触发生成）、缓存目录内文件数量未增加（未落盘垃圾缓存）。额外用`_GAN_ORDER`/`_ZHI_ORDER`穷举验证`_is_valid_ganzhi()`对全部120种天干地支组合的判定结果恰好60个`True`，且与已知60甲子顺序序列（甲子/乙丑/丙寅...）逐条比对完全吻合。`python3 -m py_compile island_service/gemini_analysis.py island_service/main.py`通过。`claude-docs/已知问题与修复记录.md`已追加本条记录（订正上一行"按日期缓存"这个已证明错误的设计描述）。**待测试**——本地无真实`GEMINI_API_KEY`，未做真实Gemini调用端到端验证，止于mock层面确认缓存key/校验逻辑正确；本次改动前该接口无前端调用方，改动后前端接入时若消费`date`字段需改为消费`ganzhi`字段，交由后续接入该端点的子agent/qa-reviewer核对 |
---
