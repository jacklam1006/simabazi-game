...
**最终要上架 App Store**，不是只做网页/PWA。这个目标对现有架构有实质约束，任何子agent做设计决策时都要考虑：

1. **上架路径**：现有前端是纯静态HTML+JS（无构建工具，CDN加载Three.js），最现实的路径是用 **Capacitor/Cordova** 把现有网页包壳成原生App，而不是重写。但苹果审核指南 **4.2（Minimum Functionality）** 会拒绝"只是套壳网站"的App——`frontend-3d`/`user-system` 在设计新交互时，倾向于增加原生感体验（离线可用、原生分享、推送通知等），而不是假设"浏览器能跑就行"
2. **内容合规**：八字/命理属于占卜类内容，苹果审核可能要求"仅供娱乐"免责声明——`user-system`/`i18n` 涉及产品文案时要保留或补充这类声明，不要为了"更专业/更玄学"而删掉免责措辞
3. **未来付费功能**：如果做会员/付费解锁，iOS端必须走 **Apple IAP**（不能直接用 Supabase/Stripe 之类外部支付完成App内计费，否则会被拒审）——`backend-service` 设计计费相关功能时要预留 IAP 校验的接口位置，不要把计费逻辑和支付方式强绑定在一起
4. **devops 领域**：后续需要评估 Capacitor 接入方案，作为与现有 Vercel/Render 部署**并存**的独立打包路径（不是替代现有网页部署）
